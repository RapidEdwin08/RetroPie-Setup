...
function depends_borked3ds() {
	...
	if isPlatform "aarch64"; then
		...
		# robin-map-dev is included the source and found when using https://github.com/rtiangha/Borked3DS.git
		depends+=(libfdk-aac-dev)
	fi
	# libqt6core6 package not in Trixie for 64
	if [[ ! $(apt-cache search libqt6core6t64) == '' ]]; then
...
function remove_borked3ds() {
    rm -f /usr/share/applications/Borked3DS.desktop
    rm -f "$home/Desktop/Borked3DS.desktop"
