...
function game_data_godot-engine-v4() {
    # get Portal2D (freeware game data)
    if [[ ! -f "$romdir/godot-engine/Portal 2D.pck" ]]; then
        mkdir -p /dev/shm/portal2d
        download "https://github.com/JulianGaibler/portal2d/releases/download/v1.0.0/Portal2D.dmg" "/dev/shm/portal2d"
        pushd /dev/shm/portal2d
        7z x /dev/shm/portal2d/Portal2D.dmg -aoa
        mv "/dev/shm/portal2d/Portal 2D/Portal 2D.app/Contents/Resources/Portal 2D.pck" "$romdir/godot-engine/Portal 2D.pck"
        popd
        rm -fr /dev/shm/portal2d
