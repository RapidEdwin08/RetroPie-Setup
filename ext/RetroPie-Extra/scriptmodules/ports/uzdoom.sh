...
#rp_module_repo="git https://github.com/UZDoom/UZDoom.git trunk 5104cc43"
...
function sources_uzdoom() {
    ...
    #applyPatch "$md_data/01_EndoomOff.diff" # 5104cc43
...
function build_uzdoom() {
    ...
    cd "$md_build/build"
    local params=(-DCMAKE_INSTALL_PREFIX="$md_inst" -DPK3_QUIET_ZIPDIR=ON -DCMAKE_BUILD_TYPE=Debug -DDYN_OPENAL=ON -DCMAKE_EXPORT_COMPILE_COMMANDS=ON -DBUILD_SHARED_LIBS=OFF -G Ninja)
