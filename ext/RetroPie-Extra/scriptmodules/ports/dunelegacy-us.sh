...
rp_module_id="dunelegacy-us"
rp_module_desc="Enhanced Port for Dune II The Building of a Dynasty (US)"
rp_module_help="Place Dune II Data Files in [ports/dune2/dunelegacy-us]:\n $romdir/ports/dune2/dunelegacy-us"
...
function depends_dunelegacy-us() {
...
function sources_dunelegacy-us() {
    gitPullOrClone
    download "https://raw.githubusercontent.com/RapidEdwin08/RetroPie-Setup/master/ext/RetroPie-Extra/scriptmodules/ports/$md_id/DuneII_48x48.svg" "$md_build"
}

function build_dunelegacy-us() {
...
function install_dunelegacy-us() {
    make install
    md_ret_files=(
        'dunelegacy.svg'
        'DuneII_48x48.svg'
...
function game_data_dunelegacy-us() {
...
function remove_dunelegacy-us() {
    if [[ -f "/usr/share/applications/Dune II The Building of a Dynasty.desktop" ]]; then sudo rm -f "/usr/share/applications/Dune II The Building of a Dynasty.desktop"; fi
    if [[ -f "$home/Desktop/Dune II The Building of a Dynasty.desktop" ]]; then rm -f "$home/Desktop/Dune II The Building of a Dynasty.desktop"; fi
    if [[ -f "$romdir/ports/Dune II The Building of a Dynasty.sh" ]]; then rm "$romdir/ports/Dune II The Building of a Dynasty.sh"; fi
}

function configure_dunelegacy-us() {
    mkRomDir "ports/dune2/$md_id"
    ...
    addPort "$md_id" "$md_id" "Dune II The Building of a Dynasty" "$launch_prefix$md_inst/bin/dunelegacy"
	if [[ ! $(dpkg -l | grep qjoypad) == '' ]]; then
		addPort "$md_id+qjoypad" "$md_id" "Dune II The Building of a Dynasty" "$launch_prefix$md_inst/$md_id-qjoy.sh"
	fi
...
_EOF_
    ...
    cat >"$md_inst/Dune II The Building of a Dynasty.desktop" << _EOF_
[Desktop Entry]
Name=Dune II The Building of a Dynasty
GenericName=Dune II The Building of a Dynasty
Comment=Dune II The Building of a Dynasty
Exec=$md_inst/bin/dunelegacy
Icon=$md_inst/dunelegacy.svg
Terminal=false
Type=Application
Categories=Game;Emulator
Keywords=D2;DuneII;Building;Dynasty
StartupWMClass=Dune II The Building of a Dynasty
Name[en_US]=Dune II The Building of a Dynasty
_EOF_
    chmod 755 "$md_inst/Dune II The Building of a Dynasty.desktop"
    if [[ -d "$home/Desktop" ]]; then cp "$md_inst/Dune II The Building of a Dynasty.desktop" "$home/Desktop/Dune II The Building of a Dynasty.desktop"; chown $__user:$__user "$home/Desktop/Dune II The Building of a Dynasty.desktop"; fi
    mv "$md_inst/Dune II The Building of a Dynasty.desktop" "/usr/share/applications/Dune II The Building of a Dynasty.desktop"
...
_EOF_
    ...
    [[ "$md_mode" == "install" ]] && game_data_dunelegacy-us
    [[ "$md_mode" == "remove" ]] && remove_dunelegacy-us
}
