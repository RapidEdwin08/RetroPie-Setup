...
function depends_dsda-doom() {
    ...
    if [[ $(apt-cache search libfluidsynth3) == '' ]]; then
        local depends=(cmake libsdl2-dev libsdl2-image-dev libsdl2-mixer-dev libogg-dev libmad0-dev libvorbis-dev libzip-dev zlib1g-dev zipcmp zipmerge ziptool libfluidsynth-dev fluidsynth libfluidsynth1)
    else
        local depends=(cmake libsdl2-dev libsdl2-image-dev libsdl2-mixer-dev libogg-dev libmad0-dev libvorbis-dev libzip-dev zlib1g-dev zipcmp zipmerge ziptool libfluidsynth-dev fluidsynth libfluidsynth3)
    fi
    getDepends "${depends[@]}"
}
