...
_EOF_
    ...
    if [[ $(cat /opt/retropie/configs/all/emulators.cfg | grep -q 'psp_StartPPSSPP = "ppsspp"' ; echo $?) == '1' ]]; then echo 'psp_StartPPSSPP = "ppsspp"' >> /opt/retropie/configs/all/emulators.cfg; chown $__user:$__user /opt/retropie/configs/all/emulators.cfg; fi

    [[ "$md_mode" == "remove" ]] && remove_ppsspp
    [[ "$md_mode" == "remove" ]] && return
    [[ "$md_mode" == "install" ]] && shortcuts_icons_ppsspp
}

function shortcuts_icons_ppsspp() {
